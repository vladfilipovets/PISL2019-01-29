...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for (int i = 0; i < letters.length; i++) {
            if (!count.containsKey(letters[i])) {
                ...
                count.replace(letters[i], count.get(letters[i])+1);

            }
        }
        //2. перенесем все символы в приоритетную очередь в виде листьев
        PriorityQueue<Node> priorityQueue = new PriorityQueue<>();
        for (Map.Entry<Character, Integer> obj : count.entrySet()) {
            priorityQueue.add(new LeafNode(obj.getValue(), obj.getKey()));
        }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        File f = new File( "D:\\Games\\ooooo\\ПИСЛ\\PISL2019-01-29\\src\\by\\it\\group673601\\zhelneruk\\lesson03\\dataHuffman.txt");
